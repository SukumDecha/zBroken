...
public class CC {
    ...
    private static final ZBroken plugin = ZBroken.get();
    ...
    public static void log(Level log, String... messages) {
        for(String message : messages) {
            ConsoleCommandSender console = Bukkit.getConsoleSender();

            String prefix = "&e[&6Log&e] &7[" + log.getColor().toString() +  log.getName() + "&7] &r";

            switch (log) {
                case CONSOLE: {
                    console.sendMessage(translate(prefix + message));
                    break;
                }
                case INFO: {
                    console.sendMessage(translate(prefix + message));
    ...
    public enum Level {
        ...
        Level(String name, ChatColor color, int level) {
            this.name = name;
            this.color = color;
            this.level = level;
        }
